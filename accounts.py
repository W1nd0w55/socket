...
def main(user_data):
    ...
    if user_data.split('&')[0].replace('method=', '') == 'signup': sign_up(user_data)
    elif user_data.split('&')[0].replace('method=', '') == 'lname': name_login(user_data)
    elif user_data.split('&')[0].replace('method=', '') == 'lemail': email_login(user_data)
...
def sign_up(user_data):
    ...
    sqlconnect.sign_up(sqlconnect.conn, username, password, email)

def name_login(user_data):
    # for logging in with the username
    username = user_data.split('&')[1].replace('username=', '')
    password = str(symbols.sha256_hash(user_data.split('&')[2].replace('password=', '')))
    sqlconnect.name_login(sqlconnect.conn, username, password)

def email_login(user_data):
    # for logging in with the email
    email = user_data.split('&')[1].replace('email=', '')
    password = str(symbols.sha256_hash(user_data.split('&')[2].replace('password=', '')))
    sqlconnect.email_login(sqlconnect.conn, email, password)
